...
if(is_null ($user1) ){//is_null — Проверяет, равно ли значение переменной null
	echo "<script>alert('Пользователь не найден'); location.href = 'auto.php' </script>";
	exit();
}
else if(count($user1) == 1){
	echo "<script>alert('Логин или пароль введены неверно'); location.href = 'auto.php' </script>";
